...
void CPlayer::UpdateCamera()
{
	...
	if (physEnt)
	{
		...
		if (physEnt->GetStatus(&status))
		{
			status.camOffset += vectorf(Legacy::from(m_walkParams.shakeLOffset));
			camera->SetCamOffset(Legacy::to(status.camOffset));//Vec3d(0, 0, status.eye_height));
		}
	...
	//	if (IsAlive())
	{
		...
		else
		{
			...
			else
			{
				...
				camera->SetAngles(Legacy::to(m_vEyeAngles));
			}
		}
	}
