...
#define DEFAULT_PORT "27015"
	...
	void Socket::Send(char* data, size_t len)
	{
		...
		auto            size = m_sendBuffer.GetSize();

		CDefaultStreamAllocator sa;
		CStream                 stm(len + size * 8, (BYTE*)data);

		//m_sendBuffer.Resize(size + len);
		stm.Write(m_sendBuffer);
		stm.WriteData(data, len);
		m_sendBuffer = stm;
		//memcpy(&m_sendBuffer.GetPtr()[size], data, len);
	}
