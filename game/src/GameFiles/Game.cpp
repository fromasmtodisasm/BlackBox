...
namespace gui
{
	...
	template<typename T>
	ImVec2& operator*(const ImVec2& l, T v)
	{
		return ImVec2(l.x * v, l.y * v);
	}
	...
	inline bool RadioButtonLabeled(const char* label, bool active, bool disabled)
	{
		...
		if (w == window->ItemWidthDefault)
			w = 0.0f; // no push item width
		...
		const ImRect check_bb(
			window->DC.CursorPos,
			ImVec2(window->DC.CursorPos + bb_size));
...
#	endif
			...
			ImGui::SameLine();
			if (RadioButtonLabeled("Standard", standardDialogMode, false))
				standardDialogMode = true;
			ImGui::SameLine();
			if (RadioButtonLabeled("Modal", !standardDialogMode, false))
				standardDialogMode = false;
			...
			{
				maxSize = ImVec2((float)gEnv->pRenderer->GetWidth() * 0.5f, (float)gEnv->pRenderer->GetHeight() * 0.5f);
			}
