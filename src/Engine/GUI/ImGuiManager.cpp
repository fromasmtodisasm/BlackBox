...
#	define MAP_BUTTON(NAV_NO, BUTTON_NO)                                                                      \
		{ io.NavInputs[NAV_NO] = (m_pInput BUTTON_NO) != 0) ? 1.0f : 0.0f; \
		}
		...
		bool OnInputEvent(const SInputEvent& event)
		{
			...
			if (event.deviceType == eIDT_Mouse)
			{
				...
				case eKI_MouseWheelUp:
					io.MouseWheel += 1; //return true;
					break;
				case eKI_MouseWheelDown:
					io.MouseWheel -= 1; //return true;
					break;
				case eKI_Mouse1:
					m_MousePressed[0] = event.state == eIS_Pressed;
					break;
				case eKI_Mouse2:
					m_MousePressed[1] = event.state == eIS_Pressed;
					break;
				case eKI_Mouse3:
					m_MousePressed[2] = event.state == eIS_Pressed;
					break;
				default:
					break;
				}
