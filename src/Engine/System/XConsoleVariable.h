...
public:
	...
	virtual const std::function<void()>& GetOnChangeFunctor(uint64 id) const override
	{
...
public:
	...
	virtual const char* GetString() const override
	{
		...
		if (type == ECVarType::Int)
			sprintf(szReturnString, "%d", m_value);
		else if (type == ECVarType::Int64)
			sprintf(szReturnString, "%lli", m_value);
		else if (type == ECVarType::Float)
			sprintf(szReturnString, "%g", m_value);
	...
	{
		CRY_ASSERT(ECVarType(GetType()) == ECVarType::String, "Wrong Set() function called. Use SetFromString() if you intended to set a numeric cvar with a string. %s", GetName());
		SetInternal<base_type>(szValue);
	}
	virtual void Set(float value) override
	{
		CRY_ASSERT(ECVarType(GetType()) == ECVarType::Float, "Wrong Set() function called. %s", GetName());
		SetInternal(static_cast<base_type>(value));
	}
	virtual void Set(int value) override
	{
		CRY_ASSERT(ECVarType(GetType()) == ECVarType::Int, "Wrong Set() function called. %s", GetName());
		SetInternal(static_cast<base_type>(value));
	}
...
public:
	...
	virtual void Set(float value) override
	{
		CRY_ASSERT(ECVarType(GetType()) == ECVarType::String, "Wrong Set() function called. %s", GetName());
	...
	virtual int GetType() const override                                        { return int(ECVarType::String); }
