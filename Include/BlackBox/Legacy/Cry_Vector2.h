...
struct Vec2_tpl
{
	...
	template<class F1>
	Vec2_tpl(const Vec2_tpl<F1>& src)
	{
		x = src.x;
		y = src.y;
	}
	template<class F1>
	explicit Vec2_tpl(const Vec3_tpl<F1>& src)
	{
		x = src.x;
		y = src.y;
	}
	template<class F1>
	explicit Vec2_tpl(const F1* psrc)
	{
		x = psrc[0];
		y = psrc[1];
	}
	...
	template<class F1>
	Vec2_tpl& operator=(const Vec2_tpl<F1>& src)
	{
		x = src.x;
		y = src.y;
		return *this;
	...
	template<class F1>
	Vec2_tpl& operator=(const Vec3_tpl<F1>& src)
	{
		x = src.x;
		y = src.y;
		return *this;
...
F1 operator*(const Vec2_tpl<F1>& op1, const Vec2_tpl<F2>& op2)
{
