...
class Profiler
{
  private:
	...
	struct GpuMarker : public Marker
	{
		...
		GpuMarker()
			: Marker()
			, id_query_start(INVALID_QUERY)
			, id_query_end(INVALID_QUERY)
		{
		}
	};
	...
	struct CpuThreadInfo
	{
		...
		void init(ThreadId id)
		{
			cur_read_id = cur_write_id = next_read_id = 0;
			thread_id								  = id;
			nb_pushed_markers						  = 0;
		}
	};
	...
	Utils::Rect m_back_rect; // Background
  ...
  public:
	...
	void pushCpuMarker(const char* name, const Utils::Color& color);
	void popCpuMarker();

	void pushGpuMarker(const char* name, const Utils::Color& color);
	void popGpuMarker();
	...
	bool isFrozen() const
	{
		return m_freeze_state == FROZEN || m_freeze_state == WAITING_FOR_UNFREEZE;
	}
