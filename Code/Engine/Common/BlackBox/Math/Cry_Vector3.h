...
public:
	...
	template<class T>
	ILINE Vec3_tpl(const Vec3_tpl<T>& v)
	{
		x = (F)v.x;
		y = (F)v.y;
		z = (F)v.z;
	...
	template<class T>
	ILINE Vec3_tpl& operator=(const Vec3_tpl<T>& v)
	{
		x = v.x;
		y = v.y;
		z = v.z;
...
template<class F>
ILINE F GetLengthSquared(const Vec3_tpl<F>& v)
{
	return v.x * v.x + v.y * v.y + v.z * v.z;
}
template<class F>
ILINE F GetLength(const Vec3_tpl<F>& v)
{
...
struct Ang3_tpl : public Vec3_tpl<F>
{
	...
	template<int SI, int SJ>
	ILINE static Ang3_tpl<F> GetAnglesXYZ(const Matrix33_tpl<F, SI, SJ>& m)
	{
		Ang3_tpl<F> a;
		a.SetAnglesXYZ(m);
		return a;
