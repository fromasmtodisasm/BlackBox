...
namespace PathUtil
{
	...
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ ToUnixPath(const TString& strPath)
	{
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ ToDosPath(const TString& strPath)
	{
		if (strPath.find('/') != TString::npos)
		{
			auto path = strPath;
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value>::type inline /*void*/ Split(const TString& filepath, TString& path, TString& filename, TString& extension)
	{
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value>::type inline /*void*/ Split(const char* szFilepath, TString& path, TString& filename, TString& extension)
	{
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value>::type inline /*void*/ Split(const TString& filepath, TString& path, TString& file)
	{
		TString extension;
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value>::type inline /*void*/ Split(const char* szFilepath, TString& path, TString& file)
	{
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ GetPathWithoutFilename(const TString& filepath)
	{
		const char* szFilepath = filepath.c_str();
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ GetFile(const TString& filepath)
	{
		const char* szFilepath = filepath.c_str();
		for (const char* p = szFilepath + filepath.length() - 1; p >= szFilepath; --p)
		{
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, void>::type inline /*void*/ RemoveExtension(TString& filepath)
	{
		const char* szFilepath = filepath.c_str();
		for (const char* p = szFilepath + filepath.length() - 1; p >= szFilepath; --p)
		{
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ RemoveExtension(const TString& filepath)
	{
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ GetFileName(const TString& filepath)
	{
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ RemoveSlash(const TString& path)
	{
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ AddSlash(const TString& path)
	{
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ AddBackslash(const TString& path)
	{
		if (path.empty() || path[path.length() - 1] == '\\')
			return path;
		if (path[path.length() - 1] == '/')
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ ReplaceExtension(const TString& filepath, const TString& extension)
	{
		auto str = filepath;
		RemoveExtension(str);
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ Make(const TString& directory, const TString& filename)
	{
	...
	//! Makes a fully specified file path from path and file name.
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ Make(const TString& directory, const TString& filename, const TString& extension)
	{
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value, TString>::type inline /*TString*/ GetParentDirectory(const TString& filePath, int generation = 1)
	{
	...
	{
#define CRY_SIMPLIFY_REJECT \
	{                       \
		*szBuf = 0;         \
		return false;       \
	}
#define CRY_SIMPLIFY_EMIT(expr) \
	{                           \
		if (--pOut < szBuf)     \
		{                       \
			*szBuf = 0;         \
...
#undef CRY_SIMPLIFY_EMIT
	...
	template<typename TString>
	typename std::enable_if<detail::IsValidStringType<TString>::value>::type inline /*void*/ UnifyFilePath(TString& path)
	{
		path.replace('\\', '/');
		path.MakeLower();
	}
