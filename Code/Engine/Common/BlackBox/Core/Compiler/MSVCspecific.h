...
#define BB_COMPILER_VERSION (_MSC_VER)
#if BB_COMPILER_VERSION < 1700
	#error This version of MSVC is not supported
#elif BB_COMPILER_VERSION < 1900
...
//! Compiler features
	...
	#define BB_COMPILER_EXCEPTIONS 1
#endif
#if defined(_CPPRTTI)
	#define BB_COMPILER_RTTI 1
#endif

//! __FUNC__ is like __func__, but it has the class name
#define __FUNC__                    __FUNCTION__
#define BB_FUNC_HAS_SIGNATURE       0

//! PREfast helpers
#define PREFAST_SUPPRESS_WARNING(W) __pragma(warning(suppress \
	                                                 : W))
#ifdef _PREFAST_
    ...
    //#define BB_DEPRECATED(message) [[deprecated(message)]] // This declaration still doesn't compile always on C++17
	#define BB_DEPRECATED(message) __declspec(deprecated(message))
#else
	#define BB_DEPRECATED(message) __declspec(deprecated(message))
#endif
...
#define BB_ALIGN(bytes) __declspec(align(bytes))
...
#define BB_FUNCTION_CONTAINS_UNDEFINED_BEHAVIOR
...
#define UNREACHABLE() __assume(0)
	...
	#define BB_DISABLE_WARN_UNUSED_VARIABLES() __pragma(warning(push)) __pragma(warning(disable : 4189))
	#define BB_RESTORE_WARN_UNUSED_VARIABLES() __pragma(warning(pop))
