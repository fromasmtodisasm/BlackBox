...
struct SWeaponHit
{
	Legacy::Vec3   pos;                  //!< Excact position of hit.
	Legacy::Vec3   normal;               //!< Normal of hitted surface.
	Legacy::Vec3   dir;                  //!< Direction of shoot.
...
public:
	...
	// positioning
	void                SetFirstPersonWeaponPos(const Legacy::Vec3& pos, const Legacy::Vec3& angles);
	//! Find position of fire for this player.
	Legacy::Vec3        GetFirePos(IEntity* pIEntity) const;

	//! Set offset of weapon for first person view.
	void                SetFirstPersonOffset(const Legacy::Vec3& posOfs, const Legacy::Vec3& angOfs);
	Legacy::Vec3        GetFirstPersonOffset() { return m_fpvPosOffset; };
	void                MoveToFirstPersonPos(IEntity* pIEntity);

	const Legacy::Vec3& GetAngles() const { return m_vAngles; }
	const Legacy::Vec3& GetPos() const { return m_vPos; }
...
private:
	...
	Legacy::Vec3      m_vPos;
	Legacy::Vec3      m_fpvPos;
	Legacy::Vec3      m_fpvAngles;
	Legacy::Vec3      m_fpvPosOffset;
	Legacy::Vec3      m_fpvAngleOffset;
...
public:
	...
	int                                Fire(const Legacy::Vec3& origin, const Legacy::Vec3& angles, CPlayer* pPlayer, WeaponInfo& winfo, IPhysicalEntity* pIRedirected);
	void                               ProcessHit(const SWeaponHit& hit);
	void                               CalculateWeaponAngles(BYTE random_seed, Legacy::Vec3* pVector, float fAccuracy);
