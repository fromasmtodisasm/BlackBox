...
class CTypedConstantBuffer : private NoCopy
{
  ...
  public:
	CTypedConstantBuffer()
		: m_hostBuffer(AlignHostBuffer())
	{
		ZeroStruct(m_hostBuffer);
	}
	CTypedConstantBuffer(const CTypedConstantBuffer<T, Alignment>& cb)
		: m_hostBuffer(AlignHostBuffer())
		, m_constantBuffer(nullptr)
	{
		m_hostBuffer = cb.m_hostBuffer;
	}
	CTypedConstantBuffer(CConstantBufferPtr incb)
		: m_hostBuffer(AlignHostBuffer())
	...
	static CD3DRenderer* ForceTwoPhase()
	{
		extern CD3DRenderer gRenderer;
		return &gRenderer;
	}
