...
#if !defined(LINUX)
	#ifdef ENTITYDLL_EXPORTS
		#define ENTITYDLL_API __declspec(dllexport)
	#else
...
struct IEntityCamera
{
	...
	virtual void             SetThirdPersonMode(const Legacy::Vec3& pos, const Legacy::Vec3& angles, int mode, float frameTime, float range, int dangleAmmount, IPhysicalEntity* physic, IPhysicalEntity* physicMore = NULL, I3DEngine* p3DEngine = NULL, float safe_range = 0.0f) = 0;
