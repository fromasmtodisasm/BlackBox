...
LRESULT CLUADbg::OnCreate(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	#else
	const char* szFolder = "res/scripts";
	#endif
	if (szFolder)
					...
					if (strstr(s, ".c:") == 0)
					{
						...
						if (nBracketStartPos != string::npos)
						{
							...
							if (nColonPos != string::npos)
							{
								...
								{
									m_wCallstack.SetItemText(thisItem, 0, static_cast<MyString&>(str.substr(0, nBracketStartPos - 1)));
									m_wCallstack.SetItemText(thisItem, 1, static_cast<MyString&>(str.substr(nColonPos + 1, nBracketEndPos - nColonPos - 1)));
									m_wCallstack.SetItemText(thisItem, 2, static_cast<MyString&>(str.substr(nBracketStartPos + 1, nColonPos - nBracketStartPos - 1)));
								}
							}
						}
...
void CLUADbg::ShowSelf(IScriptObject* pFrame)
{
	#if 0
	const char* pszText = NULL;

	m_wWatch.Clear();
	IScriptObject* pIScriptObj = pFrame;

	bool bVarFound = false;
	IScriptObject::Iterator iter = pIScriptObj->BeginIteration();
	while (pIScriptObj->MoveNext(iter))
	{
		if (iter.sKey)
		{
			...
			{
				SmartScriptObject pIEntry(m_pScriptSystem, true);
...
void CLUADbg::ShowLocalsForFrame(int frame)
{
	...
	m_nodeLookupByName.clear();
	m_wLocals.Clear();
	IScriptObject* pFrame = m_pScriptSystem->GetLocalVariables(frame/*, false*/);
	...
	if (pFrame)
	{
		...
		m_iRecursionLevel = 0;
		m_pIVariable = pFrame;
		pFrame->Dump((IScriptObjectDumpSink*) this);
		ShowSelf(pFrame);
		pFrame->Release();
	}
	else
...
HTREEITEM CLUADbg::AddVariableToTree(const char* sName, ScriptVarType _type, HTREEITEM hParent)
{
	char szBuf[2048];
	char szType[32];
...
void CLUADbg::OnElementFound(const char* sName, ScriptVarType _type)
{
	...
	if (hRoot == NULL)
	{
		hRoot = AddVariableToTree(sName, _type, m_hRoot);
...
void CLUADbg::AddBreakPoint(const char* sSourceFile, int nLine, bool bTracepoint, bool bSave)
{
	...
	sourceFile.replace("@", "");
	#else
	for (auto it = sourceFile.find("@"); it != sourceFile.npos; it = sourceFile.find(it, sourceFile.npos))
	{
		it = '\0';
	}
	#endif
