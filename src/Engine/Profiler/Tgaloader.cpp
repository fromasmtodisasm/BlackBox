...
#	endif
	...
	if (data[2] == 2 && (bpp == 3 || bpp == 4))
	{
		for (i = 0; i < width * height * bpp; i += bpp)
		{
			this->data[i + 2] = data[pos_data];
			pos_data++; // R
			this->data[i + 1] = data[pos_data];
			pos_data++; // G
			this->data[i] = data[pos_data];
			pos_data++; // B
			if (bpp == 4)
			{
				this->data[i + 3] = data[pos_data];
				pos_data++;
			} // A
		}
	}
	...
	else if (data[2] == 10 && (bpp == 3 || bpp == 4))
	{
		...
		for (i = 0; i < width * height * bpp; i += nb_pixels * bpp)
		{
			...
			else
			{
				for (j = 0; j < nb_pixels * bpp; j += bpp)
				{
					this->data[i + j + 2] = data[pos_data];
					pos_data++; // B
					this->data[i + j + 1] = data[pos_data];
					pos_data++; // G
					this->data[i + j] = data[pos_data];
					pos_data++; // R
					if (bpp == 4)
					{
						this->data[i + j + 3] = data[pos_data];
						pos_data++;
					} // A
				}
			}
