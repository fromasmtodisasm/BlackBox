...
public:
	...
	BannedID(const unsigned char *vByteArr, unsigned char bArrSize, const string &szPlayerName) { memset(vBanID, 0, 64); memcpy(vBanID, vByteArr, std::min(bArrSize, (uchar)64)); szName = szPlayerName; bSize = bArrSize; };
