...
{
	CChunkSizeProps() : m_nCount(0), m_nSize(0)
	{
	}
	CChunkSizeProps(unsigned nSize) : m_nCount(1), m_nSize(nSize)
	{
	}
...
class TGeomSorter
{
  ...
  public:
	TGeomSorter(const TVector* pVerts, int nVerts) : m_nVerts(nVerts),
													 m_pVertices(pVerts)
	{
	}
	...
	void FixSortOrder()
	{
		...
		{
			if (m_arrSortOrder[i] < 0)
				m_arrSortOrder[i] = 0;
			if (m_arrSortOrder[i] >= nComponents)
				m_arrSortOrder[i] = nComponents - 1;
		}
	}
	...
	bool operator()(int nLeft, int nRight) const
	{
		...
		for (int i = 0; i < nComponents; ++i)
		{
			...
			if (unpackComponent(m_pVertices[nLeft], nCoord) < unpackComponent(m_pVertices[nRight], nCoord))
				return true;
			else if (unpackComponent(m_pVertices[nLeft], nCoord) > unpackComponent(m_pVertices[nRight], nCoord))
				return false;
		}
		return true;
	}
...
{
  public:
	TVertexSorter(const TVector* pVerts, int nVerts, char* szSortOrder) : TGeomSorter<TVector, float, 3>(pVerts, nVerts)
	{
