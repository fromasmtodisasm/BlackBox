...
struct TKeyName
{
	...
	bool operator<(const TKeyName& n) const
	{
		return stricmp(key, n.key) < 0;
	}
	bool operator>(const TKeyName& n) const
	{
		return stricmp(key, n.key) > 0;
...
inline bool operator==(const char* str, const TKeyName& n)
{
	return n == str;
}
inline bool operator!=(const char* str, const TKeyName& n)
...
struct SUnicodeEvent
{
	...
	SUnicodeEvent(uint32_t inputChar = 0)
		: inputChar(inputChar)
	{
	}
};
...
#define clamp_tpl(val, l, r) (std::max((l), std::min((val), (r))))
	...
	ILINE float GetClampedRightGain() const
	{
		return clamp_tpl(rightGain, 0.0f, 1.0f);
	}
  ...
  private:
	void EnableFlag(Flags::Value f)
	{
		flags |= f;
	}
	void DisableFlag(Flags::Value f)
	{
		flags &= ~f;
	}
