...
typedef SVariationValue<Legacy::Vec3>  Vec3Variant;
...
struct ParticleParams
{
	...
	Legacy::Vec3		 vPosition;		 // Spawn position
	Legacy::Vec3		 vDirection;	 // Initial direction  (normalization not important)
	...
	Legacy::Vec3		 vColorStart;	 // Initial color
	Legacy::Vec3		 vColorEnd;		 // Final color
	...
	Legacy::Vec3 vNormal;			// Lying normal, used for physicalized particles only
	int	 iPhysMat;			// Material for physicalized particles
	Legacy::Vec3 vGravity;			// Gravity vector (wind)
	Legacy::Vec3 vSpaceLoopBoxSize; // if PART_FLAG_SPACELOOP is set, this box will be used to loop particles in it
};
...
struct IParticleEffect : public _i_reference_target_t
{
	...
	// Summary:
	//     Spawn the particles
	virtual void Spawn(const Legacy::Vec3& pos, const Legacy::Vec3& dir, float fScale = 1.0f) = 0;
...
struct IParticleEmitter : public _i_reference_target_t
{
	...
	virtual void SetPos(const Legacy::Vec3& vPos, const Legacy::Vec3& vDir, float fScale) = 0;
...
{
	//DOC-IGNORE-BEGIN
	virtual void	  UpdatePoints(const Legacy::Vec3* pPoints, int nCount, float fHeight) = 0;
	...
	virtual void SetPositionOffset(const Legacy::Vec3d& vNewOffset) = 0;
};
...
//DOC-IGNORE-END
	...
	//     be returned.
	virtual float GetWaterLevel(const Legacy::Vec3 * pvPos = NULL, Legacy::Vec3 * pvFlowDir = NULL) = 0;
	...
	virtual float GetWaterLevel(IEntityRender * pEntityRender, Legacy::Vec3 * pvFlowDir = NULL) = 0;
	...
	virtual void DeleteDecalsInRange( Legacy::Vec3 vBoxMin, Legacy::Vec3 vBoxMax, bool bDeleteBigTerrainDecals = true)=0;
...
//DOC-IGNORE-END
	...
	virtual Legacy::Vec3 GetWorldColor(bool bScaled=true)=0; // for example red at evening
  
	// Summary:
	//     Set the world color 
	virtual void SetWorldColor(Legacy::Vec3 vColor)=0;

	// Summary:
	//     Set the current outdoor ambient color 
	virtual void SetOutdoorAmbientColor(Legacy::Vec3 vColor)=0;
...
//DOC-IGNORE-END
	...
	virtual void OnExplosion(Legacy::Vec3 vPos, Legacy::Vec3 vHitDir, float fRadius, int nTexID, bool bDeformTerrain = true) = 0;

//DOC-IGNORE-BEGIN
    // Not used anymore
	//! Makes 3d waves on the water surface
	virtual void AddWaterSplash (Legacy::Vec3 vPos, enum eSplashType eST, float fForce, int Id=-1) = 0;
//DOC-IGNORE-END
...
//DOC-IGNORE-END
	...
	// Return Value:
	//   An rgb value contained in a Legacy::Vec3 object.
	virtual Legacy::Vec3 GetOutdoorAmbientColor() = 0;
	...
	// Return Value:
	//   An rgb value contained in a Legacy::Vec3 object.
	virtual Legacy::Vec3 GetSunColor() = 0;

	//! check object visibility taking into account portals and terrain occlusion test
	//  virtual bool IsBoxVisibleOnTheScreen(const Legacy::Vec3 & vBoxMin, const Legacy::Vec3 & vBoxMax, OcclusionTestClient * pOcclusionTestClient = NULL)=0;
	//! check object visibility taking into account portals and terrain occlusion test
	//  virtual bool IsSphereVisibleOnTheScreen(const Legacy::Vec3 & vPos, const float fRadius, OcclusionTestClient * pOcclusionTestClient = NULL)=0;
...
//DOC-IGNORE-END
	...
	virtual void ActivatePortal(const Legacy::Vec3 &vPos, bool bActivate, IEntityRender *pEntity) = 0;
...
//mat: todo
	...
	//   bUseDeepness -
	//   bUseInIndoors -
	virtual void UpdateVisArea(IVisArea * pArea, const Legacy::Vec3 * pPoints, int nCount, const char * szName, float fHeight, const Legacy::Vec3 & vAmbientColor, bool bAfectedByOutLights, bool bSkyOnly, const Legacy::Vec3 & vDynAmbientColor, float fViewDistRatio, bool bDoubleSide, bool bUseDeepness, bool bUseInIndoors) = 0;
...
//mat: todo
	...
	//! set wind direction and force Legacy::Vec3(0,0,0) = no wind
	virtual void SetWindForce( const Legacy::Vec3 & vWindForce ) = 0;
	...
	virtual	IVisArea * GetVisAreaFromPos(const Legacy::Vec3 &vPos) = 0;	
...
//DOC-IGNORE-END
  ...
  virtual void CheckPhysicalized(const Legacy::Vec3 & vBoxMin, const Legacy::Vec3 & vBoxMax) = 0;
