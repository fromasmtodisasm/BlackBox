...
//////////////////////////////////////////////////////////////////////
// searches the given entry in the map by key, and if there is none, returns the
// default value
#ifdef WIN64
#include <map>
#define hash_map map
template <typename Map, typename key_type, typename mapped_type>
inline mapped_type find_in_map(const Map& mapKeyToValue, key_type key,
                               mapped_type valueDefault)
#else
#if defined(LINUX)
...
inline typename Map::mapped_type find_in_map(
    const Map& mapKeyToValue, typename Map::key_type key,
    typename Map::mapped_type valueDefault)
#endif
...
//////////////////////////////////////////////////////////////////////
// searches the given entry in the map by key, and if there is none, returns the
// default value The values are taken/returned in REFERENCEs rather than values
#ifdef WIN64
template <typename Map, typename key_type, typename mapped_type>
inline mapped_type& find_in_map_ref(Map& mapKeyToValue, key_type key,
                                    mapped_type& valueDefault)
#else
template <typename Map>
inline typename Map::mapped_type& find_in_map_ref(
    Map& mapKeyToValue, typename Map::key_type key,
    typename Map::mapped_type& valueDefault)
#endif
{
...
unsigned sizeofArray(const Container& arr, unsigned nSize) {
  return arr.empty()
             ? 0u
             : (unsigned)(sizeof(typename Container::value_type) * nSize);
}
...
unsigned countElements(const std::vector<T>& arrT, const T& x) {
  unsigned nSum = 0;
  for (typename std::vector<T>::const_iterator iter = arrT.begin();
       iter != arrT.end(); ++iter)
    if (x == *iter) ++nSum;
  return nSum;
}
...
namespace stl {
//////////////////////////////////////////////////////////////////////////
//! Searches the given entry in the map by key, and if there is none, returns
//! the default value
//////////////////////////////////////////////////////////////////////////
#ifdef WIN64
template <typename Map, typename key_type, typename mapped_type>
...
inline void map_to_vector(const Map& theMap, Vector& array) {
  ...
  for (typename Map::const_iterator it = theMap.begin(); it != theMap.end();
       ++it) {
...
inline void set_to_vector(const Set& theSet, Vector& array) {
  ...
  for (typename Set::const_iterator it = theSet.begin(); it != theSet.end();
       ++it) {
    array.push_back(*it);
  }
}
...
//////////////////////////////////////////////////////////////////////////
// Hash map usage:
// typedef stl::hash_map<string,int, stl::hash_stricmp<string> >
// StringToIntHash;
//////////////////////////////////////////////////////////////////////////
