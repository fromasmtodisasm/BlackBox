...
LastCommand::LastCommand(CGame* game)
	: BaseGameCommand(game)
{
}
...
ClearCommand::ClearCommand(CGame* game)
	: BaseGameCommand(game)
{
}
...
GotoCommand::GotoCommand(CGame* game)
	: BaseGameCommand(game)
{
}
//*******************************************************
...
QuitCommand::QuitCommand(CGame* game)
	: BaseGameCommand(game)
{
}
//*******************************************************
...
MoveCommand::MoveCommand(CGame* game)
	: BaseGameCommand(game)
{
	...
	virtual bool execute(CommandDesc& cd) override
	{
		if (cd.args.size() >= 4 && cd.args.size() <= 5)
		{
			...
			if (obj != nullptr)
			{
				...
				if (vector[0] != 0.0f)
					angles[0] += obj->m_transform.rotation[0];
				if (vector[1] != 0.0f)
					angles[1] += obj->m_transform.rotation[1];
				if (vector[2] != 0.0f)
					angles[2] += obj->m_transform.rotation[2];
...
RotateCommand::RotateCommand(CGame* game)
	: BaseGameCommand(game)
{
	m_World = game->getWorld();
}
...
SelectCommand::SelectCommand(CGame* game)
	: BaseGameCommand(game)
{
	m_World = game->getWorld();
}
//*******************************************************
class ExecCommand : public BaseGameCommand
	, public IWorkerCommand
{
	IWorld* m_World;
	IConsole* console;
	int wait_cnt = 0;
...
ExecCommand::ExecCommand(CGame* game)
	: BaseGameCommand(game)
{
	m_World = game->getWorld();
...
CameraCommand::CameraCommand(CGame* game)
	: BaseGameCommand(game)
{
	m_World = game->getWorld();
}
...
SceneCommand::SceneCommand(CGame* game)
	: BaseGameCommand(game)
{
	m_World = game->getWorld();
}
...
TagPointCommand::TagPointCommand(CGame* game)
	: BaseGameCommand(game)
{
	m_World = game->getWorld();
}
...
ObjDumpCommand::ObjDumpCommand(CGame* game)
	: BaseGameCommand(game)
{
	m_World = game->getWorld();
}
